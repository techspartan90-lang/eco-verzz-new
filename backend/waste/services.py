...
class AIWasteDetectionService:
    ...
    def detect_waste_type(image_file):
        ...
        if "plastic" in filename or "bottle" in filename:
            ...
            confidence = 0.94
            recyclable = True
            suggestions = ["Deposit in yellow recycling bin.",
            ...
            confidence = 0.95
            recyclable = True
            suggestions = ["Separate by color if possible.",
                           "Avoid breaking glass before disposal."]
        elif "food" in filename or "organic" in filename or "leaf" in filename:
            category = "ORGANIC"
            confidence = 0.88
            recyclable = False
            suggestions = ["Ideal for composting.",
                           "Keep organic waste separate to prevent odor."]
        elif "battery" in filename or "phone" in filename or "wire" in filename or "electronics" in filename:
            category = "E_WASTE"
            confidence = 0.97
            recyclable = False
            suggestions = ["Contains hazardous materials.",
                           "Must be taken to an official e-waste drop-off site."]
        else:
            category = "OTHER"
            confidence = 0.75
            recyclable = False
